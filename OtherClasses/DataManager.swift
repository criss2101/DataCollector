...
class DataManager
{
    ...
    static func connectSensorsDataAndSaveAll(fileName: String, iphoneData: [SensorData], watchData: [SensorData])
    {
        ...
        for ind  in 0...count-1
        {
            ...
            csvString.append("\(id),\(iphoneData[ind+offset].timeStamp),\(iphoneData[ind+offset].gravData.x),\(iphoneData[ind+offset].gravData.y),\(iphoneData[ind+offset].gravData.z),\(iphoneData[ind+offset].userAccData.x),\(iphoneData[ind+offset].userAccData.y),\(iphoneData[ind+offset].userAccData.z),\(iphoneData[ind+offset].attData.x),\(iphoneData[ind+offset].attData.y),\(iphoneData[ind+offset].attData.z),\(iphoneData[ind+offset].rotRateData.x),\(iphoneData[ind+offset].rotRateData.y),\(iphoneData[ind+offset].rotRateData.z),\(watchData[ind].timeStamp),\(watchData[ind].gravData.x),\(watchData[ind].gravData.y),\(watchData[ind].gravData.z),\(watchData[ind].userAccData.x),\(watchData[ind].userAccData.y),\(watchData[ind].userAccData.z),\(watchData[ind].attData.x),\(watchData[ind].attData.y),\(watchData[ind].attData.z),\(watchData[ind].rotRateData.x),\(watchData[ind].rotRateData.y),\(watchData[ind].rotRateData.z)\n")
            id+=1
        }
